...
// FIX: Import all necessary types for the data fetching and props.
...
// FIX: Define an interface for all the data needed by the admin console and its children.
interface AdminData {
  products: Product[];
  bookings: Booking[];
...
export const AdminConsole: React.FC = () => {
  ...
  // FIX: Add state to manage data fetching and loading states.
  const [dataVersion, setDataVersion] = useState(0);
  const [isLoading, setIsLoading] = useState(true);
  const [adminData, setAdminData] = useState<AdminData | null>(null);

  // FIX: Create a centralized function to fetch all required data.
  ...
  // FIX: Fetch data on component mount and when dataVersion changes (on sync).
  useEffect(() => {
    fetchData();
  }, [dataVersion, fetchData]);
  
  // FIX: Removed obsolete `useEffect` that was listening for storage events.
  
  // FIX: handleSync now triggers a data refresh by incrementing dataVersion.
  ...
  // FIX: Make the function async to await the promise from getBookings().
  const handleNotificationClick = async (notification: Notification) => {
    handleSync(); // Refresh data before navigating
  ...
  const renderContent = () => {
    // FIX: Add a loading state while fetching initial data.
    ...
    // FIX: Create a complete AppData object for ScheduleManager.
    const appDataForScheduleManager: AppData = { 
        ...adminData, 
        policies: '', // Not needed in ScheduleManager
        confirmationMessage: { title: '', message: ''}, // Not needed
        footerInfo: { address: '', email: '', whatsapp: '', googleMapsLink: '', instagramHandle: '' } // Not needed
    };

    switch (activeTab) {
      case 'products':
        // FIX: Pass required props to ProductManager.
        return <ProductManager products={adminData.products} onDataChange={handleSync} />;
      case 'calendar':
        if (calendarView === 'month') {
          // FIX: Pass required props to CalendarOverview.
          return <CalendarOverview bookings={adminData.bookings} onDateSelect={handleDateSelect} onDataChange={handleSync} />;
        } else {
          // FIX: Pass required props to ScheduleManager.
      ...
      case 'schedule-settings':
        // FIX: Pass required props to ScheduleSettingsManager.
        ...
        />;
      case 'financials':
        // FIX: Pass required props to FinancialDashboard.
        return <FinancialDashboard bookings={adminData.bookings} onDataChange={handleSync}/>;
      case 'customers':
        // FIX: Pass required props to CrmDashboard.
        return <CrmDashboard bookings={adminData.bookings} navigateToEmail={targetId} />;
      case 'inquiries':
        // FIX: Pass required props to InquiryManager.
      ...
      case 'settings':
        return <SettingsManager />; // Settings manager handles its own data fetching.
      default:
        return null;
    }
  };
      ...
      <div className="bg-brand-background min-h-screen text-brand-text font-sans">
        ...
        <header className="bg-brand-surface shadow-md">
          <div className="container mx-auto px-4 py-4 flex items-center justify-between">
            ...
            <div className="w-24 flex justify-end items-center gap-2">
                {/* FIX: Removed `hasNewData` prop as it's no longer used. */}
        ...
        <main className="container mx-auto px-4 py-8">
          <div className="max-w-screen-xl mx-auto">
            ...
            {/* FIX: Removed key prop; re-renders are handled by state updates. */}
